...
from common import (CACHE, Fetcher, digital_object_uris, gallery, in_parallel,  # noqa: E402
                    visual_item_uri)
...
def main() -> None:
    ...
    for record in in_parallel(candidates, resolve, workers=args.workers, label="records"):
        if record and gallery(record):
            on_view.append(record)

    print(f"{len(on_view)}/{len(candidates)} on view "
          f"(cache {fetcher.hits} hit / {fetcher.misses} miss)", file=sys.stderr)

    # Second hop: only on-view objects need their imagery resolved.
    visual_uris = [uri for record in on_view if (uri := visual_item_uri(record))]
